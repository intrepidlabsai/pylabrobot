...
# Live firmware we brought up lists ``clearabort, ca`` instead of
# ``clearbuttonabort, cba``. Map those names onto the same handler so the
# backend's ``ca`` wire command works against the TCP mock.
_ALIAS_MAP.setdefault("ca", "clearbuttonabort")
_ALIAS_MAP.setdefault("clearabort", "clearbuttonabort")
...
class MicroSpinMockServer:
  ...
  async def _h_version(self, args, cmd_id):
    return [
      ...
      "Serial Number: HRB-MOCK-0000001", # placeholder for real serial number
