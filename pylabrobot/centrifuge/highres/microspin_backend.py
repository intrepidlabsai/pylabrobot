...
from pylabrobot.centrifuge.highres.tcp import Socket
...
class MicroSpinBackend(CentrifugeBackend):
  ...
  async def clear_button_abort(self) -> None:
    ...
    Sends ``ca`` (``clearabort``). This unit's ``list`` names that command;
    older HighRes docs used ``clearbuttonabort`` / ``cba``, which this
    firmware rejects with ``-15: Command not recognized``.
    """
    await self.send_command("ca")
